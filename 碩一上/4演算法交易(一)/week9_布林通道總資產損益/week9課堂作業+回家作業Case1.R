...
#0:否、1:是
data$PL1<-0#損益累計1
data$Cash1<-10000000#現金累計1、初始1000萬
data$Capital1<-10000000#資產累計1、初始1000萬
data$PL2<-0#損益累計2
...
for(m in 21:nrow(data)){
  ...
  if(!hasBuy1){
    ...
    if(data$Sign1[m-1]==0 && data$Sign1[m]==1){
      Buy1=data$Close[m]
      data$Cash1[m]=data$Cash1[m-1]-Buy1*1000
      STKValue1=Buy1*1000
      hasBuy1=TRUE
    }
  }else{
    STKValue1=data$Close[m]*1000
    #若收盤價跌破上緣賣出
    if(data$Sign1[m-1]==1 && data$Sign1[m]==0){
      Sell1=data$Close[m]
      data$PL1[m]=Sell1-Buy1
      data$Cash1[m]=data$Cash1[m-1]+Sell1*1000
      STKValue1=0
      hasBuy1=FALSE
    }
...
for(m in 21:nrow(data)){
  ...
  if(!hasBuy2){
    ...
    if(data$Sign2[m-1]==1 && data$Sign2[m]==0){
      Sell2=data$Close[m]
      data$Cash2[m]=data$Cash2[m-1]+Sell2*1000
      hasBuy2=TRUE
    }
  }else{
    STKValue2=data$Close[m]*1000
    #若收盤價突破下緣平倉
    if(data$Sign2[m-1]==0 && data$Sign2[m]==1){
      Buy2=data$Close[m]
      data$PL2[m]=Sell2-Buy2
      STKValue2=0
      data$Cash2[m]=data$Cash2[m]-Buy2*1000
      hasBuy2=FALSE
    }
...
plot(data$Capital1,type="l",lwd=2,col="red",main="資產累計1(初始1000萬)")
plot(data$Capital2,type="l",lwd=2,col="green",main="資產累計2(初始0)")
plot(data$Capital3,type="l",lwd=2,col="blue",main="資產累計3(初始1000萬)")
